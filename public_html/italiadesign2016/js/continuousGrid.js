...
$(document).ready(function() {
  ...
  ** =====================================================*/
  ...
  $window.on('resize', function() {
    $scriptEnable = $window.outerWidth() > 700 && !mobile ? true : false;
    offsetGrid();
    if($window.outerWidth() < 700) $entries.css('transform', '');
  });
  ...
  function orientationChange(mql) {
    if(mql.matches) {
      window.requestAnimationFrame(offsetGrid);
    } else {
      // bgScroll();
      if($window.outerWidth < breakpoints.tablet) window.requestAnimationFrame(offsetGrid);
    }
  }
  ...
  $window.on('grid:loaded', function() {
    ...
    //================ BACKGROUND HEIGHT ================//
    if($teamPage && $window.outerWidth >= breakpoints.laptop) {
      // bgScroll();
    }
    ...
    $window.on('scroll', function(e) {
      //Run Parallax
      if ($window.outerWidth() > breakpoints.laptop && !mobile) this.requestAnimationFrame(parallax);
