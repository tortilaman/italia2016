...
$(document).ready(function() {
    ...
    **===============================*/
    ...
    function showControls() {
        if (played === true) {
            $("#v-title, #v-interviewee, #v-share, a.v-prev-link, .v-controls, .v-nextVid, .v-prevVid, .v-gradient").removeClass("oHidden");
        }
    }

    function hideControls() {
        if (vPlayer.paused === false) {
            $("#v-title, #v-interviewee, #v-share, a.v-prev-link, .v-controls, .v-nextVid, .v-prevVid, .v-gradient").addClass("oHidden");
        }
    }
    ...
    **===============================*/
    ...
    /***************************************************************************
          ...
          d88888                   888          888    Y8P
         d88P888                   888          888
        d88P 888 88888b.   8888b.  888 888  888 888888 888  .d8888b .d8888b
       d88P  888 888 "88b     "88b 888 888  888 888    888 d88P"    88K
      d88P   888 888  888 .d888888 888 888  888 888    888 888      "Y8888b.
     d8888888888 888  888 888  888 888 Y88b 888 Y88b.  888 Y88b.         X88
    d88P     888 888  888 "Y888888 888  "Y88888  "Y888 888  "Y8888P  88888P'
                                            888
                                       Y8b d88P
                                        "Y88P"
    ***************************************************************************/

    vPlayer.addEventListener('ended', videoEnd, false);
    vPlayer.addEventListener('play', videoPlay, false);
    vPlayer.addEventListener('pause', videoPause, false);

    function videoEnd() {
      console.log("video ended");
      ga('send', 'event', 'video', '100% video played', videoTitle);
      console.log("GA: video ended");
      videoProgress();
    }

    var videoTitle = $("#v-interviewee").text(),
        watched25 = false,
        watched50 = false,
        watched75 = false;
        watched100 = false;
    ...
    function videoProgress() {
      var duration = vPlayer.duration,
          playedLen = vPlayer.played.length,
          playedTime = 0,
          playedPercent;
      for(var i = 0; i < playedLen; i++ ) {
        playedTime += (vPlayer.played.end(i) - vPlayer.played.start(i));
        playedPercent = (playedTime / duration * 100).toFixed(2);
      }
      // console.log("Played time is: "+playedTime+", Played percent is: "+playedPercent);
      if (playedPercent > 25 && watched25 === false) {
        watched25 = true;
        ga('send', 'event', 'video', '25% video watched', videoTitle);
        console.log("GA: 25% watched");
      }
      if (playedPercent > 50 && watched50 === false) {
        watched50 = true;
        ga('send', 'event', 'video', '50% video watched', videoTitle);
        console.log("GA: 50% watched");
      }
      if (playedPercent > 75 && watched75 === false) {
        watched75 = true;
        ga('send', 'event', 'video', '75% video watched', videoTitle);
        console.log("GA: 75% watched");
      }
      if (playedPercent == 100 && watched100 === false) {
        watched100 = true;
        ga('send', 'event', 'video', '100% video watched', videoTitle);
        console.log("GA: 100% watched");
      }
    }

    function videoPlay() {
      if(vPlayer.played.length === 0) {
        ga('send', 'event', 'video', 'video played', videoTitle);
        console.log("GA: Video played");
      }
    }
...
88888888 888  88888P'  "Y888 "Y8888  888  888  "Y8888  888      88888P'
*************************************************************************/
    ...
    //Stop video if you go to vimeo
    $(".v-share-vi").on('click', function() {
      if(!mobile && !vPlayer.paused) {
        playButton.toggle();
      } else {
        $vPlayer.eq(0).pause();
      }
    ...
    //Click vieo to pause.
    $('#v-player, .v-gradient').on('click', function(e) {
        playButton.toggle();
    });

    $window.on('keyup', function(e) {
      if(e.which == 32 && $('body').hasClass('home') && vPlayer.played.length === 0) {
      } else if(e.which == 32 && $('body').hasClass('home')) {
        playButton.toggle();
      }
    });
